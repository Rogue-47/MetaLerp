...
this is much faster than employing a truly dynamic and run-time flexible type-switching system
and is necessary for this lib's main goal of being very light and fast; due to this - its internal configuration
is only changeable through compilation of the source code with specific macro definitions
so basically, if really high precision (double floating-point that is) is needed, you'll have to recompile
with the flag -DTYPE_FLOAT64 in the compilation command
(check out suggested compilation commands in the build folder)

though I am thinking of making a compilation utility CLI for python module setup
with an easy interview-like procedure to determine your preferred static configuration of this lib,
just because of the amount of compile-time configurability that I have designed this library natively with.
*/
    ...
    */
    ...
    //setters are referenced from the CUDA object file
        ...
        #define METALERP_INTERNAL_KERNEL(funcName) \
            ...
            K_##funcName(const type* __restrict__ inArray, type* __restrict__ outArray, size_t len)   \
                {   \
                    size_t idx = blockIdx.x * blockDim.x + threadIdx.x; \
                    if(idx < len) \
                    {   \
                        outArray[idx] = NM(funcName)(inArray[idx]); \
                    } \
                }   \
            void deviceDispatch_##funcName(const type* __restrict__ in, type* __restrict__ out, size_t len)   \
                {   \
                    ...
                    \
                    type *D_in = NULL, *D_out = NULL;   \
                    \
                    CUDA_CALL(cudaMalloc((void**)&D_in, numBytes))  \
                    CUDA_CALL(cudaMalloc((void**)&D_out, numBytes)) \
                    \
                    CUDA_CALL(cudaMemcpy((void*)D_in, (const void*)in, numBytes, cudaMemcpyHostToDevice))   \
                    \
                    K_##funcName<<<computeGridSize(len), METALERP_BLOCK_SIZE>>>(D_in, D_out, len);  \
                    \
                    CUDA_CALL(cudaMemcpy(out, D_out, numBytes, cudaMemcpyDeviceToHost)) \
                    \
    ...
    type branchingSign(type x);
    METALERP_INTERNAL_DEVFUNC
    type nonBranchingSign(type x);
    ...
    METALERP_INTERNAL_DEVFUNC
    type branchingSign(type x) //branching sign func
    {
    ...
    METALERP_INTERNAL_DEVFUNC
    type nonBranchingSign(type x) //less branching (only the sign bias check) but heavily arithmetic
    {
    ...
    //SGN macro is the default sign function for unspecified (though with further testing might be removed)
            ...
            #define SGN NBSGN 
            #define BSGN branchingSign
            #define NBSGN nonBranchingSign
        #else
            #define SGN BSGN 
            #define BSGN branchingSign
            #define NBSGN nonBranchingSign
